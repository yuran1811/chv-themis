...
const { RANKING_MODE, PROBLEMS_DIR, RANKING_DIR, TASKS_DIR, CONTESTANTS_DIR, SUBMISSIONS_DIR, LOGS_DIR } = THEMIS_DIR;
...
export const cvertSubmissionName = (req, file, lang = 'cpp') => `[${req.params.user}][${file}].${lang}`;
...
export const getAuthStatus = (req) => cookieParser.JSONCookies(req.cookies.isAuth || 0) || 0;
export const getAuthUser = (req) => cookieParser.JSONCookies(req.cookies.user || '') || 'null';
...
export const getRankOnl = () => {
  ...
  const status = list.map((_) => {
    ...
    const list = tasks.map((task) => (_[1].hasOwnProperty(task) ? _[1][task] : 0));
